...
export class PredicteCompletionProvider implements vscode.InlineCompletionItemProvider {
    ...
    constructor(config: PredicteConfig, secretStorage: PredicteSecretStorage, logger: Logger) {
        this.config = config;
        this.logger = logger;
    ...
    async provideInlineCompletionItems(
        ...
        console.warn('[DEBUG] Position:', `line ${position.line}, char ${position.character}`);
        ...
        try {
            ...
            const completion = await this.debouncer.debounce(async () => {
                ...
                const codeContext = extractContext(document, position, this.config.contextLines);
                ...
                console.warn('[DEBUG] API call completed, result:', result ? 'received' : 'null');

                if (result) {
                    this.logger.debug(`Received completion: ${result.substring(0, 100)}...`);
            ...
            console.warn('[DEBUG] Sanitized completion:', sanitized ? 'success' : 'failed');

            // Validate completion
            console.warn('[DEBUG] Validating completion...');
            if (!isValidCompletion(sanitized)) {
                console.warn('[DEBUG] Completion invalid after sanitization, returning null');
            ...
            console.warn('[DEBUG] Returning completion item:', sanitized.substring(0, 100));
    ...
    private async getStreamingCompletion(
        ...
        try {
            ...
            for await (const chunk of this.mistralClient.getStreamingCompletion(
                ...
                chunkCount++;
                chunks.push(chunk);
                console.warn('[DEBUG] Received chunk', chunkCount, 'length:', chunk.length);
            }
            console.warn('[DEBUG] Streaming completed, total chunks:', chunkCount);

            const result = chunks.length > 0 ? chunks.join('') : null;
            console.warn('[DEBUG] Streaming result:', result ? 'has content' : 'empty');
            return result;
        } catch (error) {
    ...
    private handleError(error: unknown): null {
        ...
        if (error instanceof MistralClientError) {
            ...
            this.logger.error(`MistralClientError [${error.code}]: ${error.message}`, error.cause);
